...
export default function PhonePlans() {
    ...
    useEffect(() => {
        if (document.getElementById('phone-plan')) {
            let positionTop = document.getElementById('phone-plan').offsetTop;
            window.scrollTo({
                top: positionTop,
                behavior: 'smooth'
            });
    ...
    return (
        <Fragment>
            {phonePlans && phonePlans.length > 0 &&
                <div id="phone-plan" className="py-8 px-4 mx-auto max-w-screen-xl lg:py-16 lg:px-6">
                            ...
                            phonePlans.map(plan => {
                                return (
                                    <div key={plan.id} className="flex flex-col p-6 mx-auto max-w-lg text-center text-gray-900 bg-white rounded-lg border border-gray-100 shadow hover:drop-shadow-2xl cursor-pointer">
                                        ...
                                        {plan.mostPopular &&
                                            <div
                                                className="mb-2 rounded bg-blue-500 px-2 py-2 text-xs font-medium uppercase leading-normal text-white mr-auto ml-auto mb-5 w-1/2">
